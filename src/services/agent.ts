...
export class AgentService {
  ...
  private async requestChangesWithDiscovery(input: {
    ticket: JiraTicket;
    repoPath: string;
    ...
    for (
      ...
      input.hooks?.onProgress?.(
        `Planning changes with the agent (${round}/${this.config.OPENAI_CONTEXT_ROUNDS}).`,
      );
      ...
      const extraFiles = await loadFilesByPath(
        input.repoPath,
      ...
      input.hooks?.onProgress?.(
        `Loaded ${extraFiles.length} more file${extraFiles.length === 1 ? "" : "s"} for review.`,
      );
  ...
  private async requestAgentDecision(input: {
    ticket: JiraTicket;
    repoPath: string;
    context: RepositoryContext;
    mode: "implementation" | "repair";
    ...
    round: number;
    maxRounds: number;
  }): Promise<AgentContextRequestResponse> {
...
function parseAgentDecision(text: string): AgentContextRequestResponse | null {
